...
conv_handler = ConversationHandler(
    ...
    states={
        ...
        STATE_CONT: [CommandHandler('location', command_location_handler),
                     RegexHandler(u'^(Меню)$', start_handler),
                     MessageHandler([Filters.location], location),
                     MessageHandler([Filters.text], select_continent)],

        STATE_REGION: [RegexHandler(u'^(Назад)$', start_handler),
                    RegexHandler(u'^(Меню)$', start_handler),
                    MessageHandler([Filters.text], continent_handler)],

        STATE_TYPE: [RegexHandler(u'^(Назад)$', select_continent),
                    RegexHandler(u'^(Меню)$', start_handler),
                    MessageHandler([Filters.text], region_handler)],

        STATE_MAP:  [RegexHandler(u'^(Назад)$', continent_handler),
                    RegexHandler(u'^(Меню)$', start_handler),
                    MessageHandler([Filters.text], type_handler)],

        SHOW_MAP:   [RegexHandler(u'^(Меню)$', start_handler),
                    RegexHandler(u'^(Назад)$', region_handler),
                    RegexHandler(u'^(Обновить)$', refresh_handler),
